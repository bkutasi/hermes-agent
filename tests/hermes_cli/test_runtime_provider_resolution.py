...
def test_custom_provider_model_api_mode_overrides_default(monkeypatch):
    monkeypatch.setattr(
        rp,
        "_get_named_custom_provider",
        lambda _p: {
            ...
            "model": "default-model",
            "models": {
                "claude-glm-5.2": {"api_mode": "anthropic_messages"},
            },
        },
    )

    resolved = rp.resolve_runtime_provider(
        requested="myproxy", target_model="claude-glm-5.2"
    )
...
def test_custom_provider_pool_uses_model_api_mode(monkeypatch):
    seen = {}

    def resolve_pool(*args, **kwargs):
        seen["api_mode"] = args[2]
        return {"provider": "custom", "api_key": "pool-key", "base_url": args[0]}

    monkeypatch.setattr(rp, "_try_resolve_from_custom_pool", resolve_pool)
    monkeypatch.setattr(
        rp,
        "_get_named_custom_provider",
        lambda _p: {
...
def test_modern_custom_provider_model_api_mode_overrides_default(monkeypatch, pooled):
    seen = {}
    ...
    monkeypatch.setattr(rp.auth_mod, "resolve_provider", lambda _p: "myproxy")
    if pooled:
        def resolve_pool(*args, **kwargs):
            seen["api_mode"] = args[2]
            return {"provider": "custom", "api_key": "pool-key", "base_url": args[0]}
    ...
    resolved = rp.resolve_runtime_provider(
        requested="custom:myproxy", target_model="claude-glm-5.2"
    )
