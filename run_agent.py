...
class AIAgent:
    ...
    def _anthropic_preserve_dots(self) -> bool:
        ...
        return (
            ...
            # Custom Anthropic-compatible proxies pass model IDs through;
            # only native api.anthropic.com requires dotted Claude IDs to be
            # rewritten with hyphens.
            or (
                (getattr(self, "provider", "") or "").lower() == "custom"
                and not base_url_host_matches(base, "api.anthropic.com")
                # Do not treat an arbitrary custom AWS endpoint as an
                # Anthropic-compatible proxy; Bedrock is handled above.
