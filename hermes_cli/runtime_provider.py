...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
        ...
        for ep_name, entry in providers.items():
            ...
            if requested_norm in custom_provider_aliases(
                ...
                if base_url:
                    ...
                    # Keep model-specific transport settings available to the
                    # runtime resolver, as with legacy custom_providers.
                    models = entry.get("models")
    ...
    for entry in custom_providers:
        ...
        _lift_max_output_tokens(entry, result)
        # Keep model-specific transport settings available to the runtime
        # resolver; a custom provider can route different models via distinct
        # compatible APIs.
        models = entry.get("models")
        if isinstance(models, dict):
            result["models"] = models
...
def _resolve_named_custom_runtime(
    *,
    ...
    # Model-specific api_mode overrides the provider default. Use the model
    # being resolved, not config.model.default: auxiliary/fallback calls can
    # target a different model in the same custom provider.
    ...
    if isinstance(provider_models, dict) and model_name:
        model_config = provider_models.get(model_name)
        if isinstance(model_config, dict):
            model_api_mode = _parse_api_mode(model_config.get("api_mode"))
            if model_api_mode:
                resolved_api_mode = model_api_mode
